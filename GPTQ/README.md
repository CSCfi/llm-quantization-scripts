...
### Install packages
```bash
(venv)> pip install gptqmodel==7.1.0 dataset --no-deps --no-build-isolation --cache-dir ./.pip-cache
```
...
```bash
(venv)> pip install llmcompressor==0.7.1 --no-deps --cache-dir ./.pip-cache
(venv)> pip install wandb --no-deps --cache-dir ./.pip-cache
``` 
